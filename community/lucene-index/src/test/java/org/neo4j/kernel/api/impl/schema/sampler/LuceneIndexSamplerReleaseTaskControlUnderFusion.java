...
public class LuceneIndexSamplerReleaseTaskControlUnderFusion
{
    ...
    private static final SchemaIndexDescriptor indexDescriptor = forLabel( 1, 1 );
    private static final IndexProvider.Descriptor providerDescriptor = IndexProvider.UNDECIDED;
    ...
    public void setup()
    {
        directoryFactory = defaultDirectoryStructure( dir.graphDbDir() );
    }
    ...
    public void failedIndexSamplingMustNotPreventIndexDrop() throws IOException, IndexEntryConflictException
    {
        ...
        try ( IndexAccessor fusionAccessor = fusionProvider.getOnlineAccessor( indexId, indexDescriptor, samplingConfig ) )
        {
    ...
    {
        try ( IndexAccessor accessor = provider.getOnlineAccessor( indexId, indexDescriptor, samplingConfig );
              IndexUpdater updater = accessor.newUpdater( IndexUpdateMode.ONLINE ) )
        {
            updater.process( IndexEntryUpdate.add( 1, indexDescriptor, Values.of( "some string" ) ) );
        }
    }
    ...
    private FusionIndexProvider createFusionProvider( LuceneIndexProvider luceneProvider, IndexProvider failingProvider )
    {
        ...
        return new FusionIndexProvider( failingProvider, EMPTY, EMPTY, EMPTY, luceneProvider, slotSelector, providerDescriptor, 1, directoryFactory, fs.get(),
                false );
    }
    ...
    {
        return new IndexProvider.Adaptor( providerDescriptor, 1, directoryFactory )
        {
            @Override

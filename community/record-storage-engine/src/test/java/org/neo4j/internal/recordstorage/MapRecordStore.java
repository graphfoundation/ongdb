...
class MapRecordStore implements LockVerificationMonitor.StoreLoader
{
    ...
    RecordAccessSet newRecordChanges( RecordAccess.LoadMonitor loadMonitor, Monitor monitor )
    {
        return new RecordChangeSet(
                ...
                loader( groupStore, ( id, type ) -> withType( new RelationshipGroupRecord( id ), type ), monitor::relationshipGroupWasRead ),
                noLoader(), noLoader(), noLoader(), noLoader(),
                EmptyMemoryTracker.INSTANCE,
                loadMonitor, StoreCursors.NULL );
    }
    ...
    void write( RelationshipGroupRecord record )
    {
        ...
        // This is a special thing where we know that the prev pointer isn't actually persisted and so that fact is mimiced here
        copy.setPrev( -1 );
    ...
    {
        return loader( LongObjectMaps.mutable.empty(), ( id, ignore ) ->
        {
            throw new IllegalStateException( "Should not be needed" );
        }, record -> {} );
    }

    private static <T extends AbstractBaseRecord,R> RecordAccess.Loader<T,R> loader( MutableLongObjectMap<T> store, BiFunction<Long,R,T> factory,
            Consumer<T> monitor )
    {

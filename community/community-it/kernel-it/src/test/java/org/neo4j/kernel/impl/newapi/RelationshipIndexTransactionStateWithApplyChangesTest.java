...
public class RelationshipIndexTransactionStateWithApplyChangesTest extends IndexTransactionStateWithApplyChangesTestBase
{
    ...
    void createIndex( IndexType indexType )
    {
        ...
        {
            tx.schema().indexFor( RelationshipType.withName( DEFAULT_TYPE ) ).on( PROP1 ).on( PROP2 ).withIndexType( indexType ).withName( INDEX_NAME )
                    .create();
            tx.commit();
        }
            ...
            Object anotherValueFoundByQuery, Object anotherValueFoundByQuery2 ) throws Exception
    {
        ...
        try ( RelationshipValueIndexCursor rels = tx.cursors().allocateRelationshipValueIndexCursor( tx.cursorContext(), tx.memoryTracker() ) )
        {
            ...
            assertEntityAndValue( expected, tx, needsValues, anotherValueFoundByQuery, anotherValueFoundByQuery2, new RelationshipCursorAdapter( rels ) );
        }
    }

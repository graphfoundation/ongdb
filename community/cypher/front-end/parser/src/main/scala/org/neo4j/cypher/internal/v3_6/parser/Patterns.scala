...
trait Patterns extends Parser
  ...
  def Pattern: Rule1[org.neo4j.cypher.internal.v3_6.expressions.Pattern] = rule("a pattern") {
    oneOrMore(PatternPart, separator = CommaSep) ~~>> (ast.Pattern(_))
  }

  def PatternPart: Rule1[org.neo4j.cypher.internal.v3_6.expressions.PatternPart] = rule("a pattern") (
      group(Variable ~~ operator("=") ~~ AnonymousPatternPart) ~~>> (ast.NamedPatternPart(_, _))
    | AnonymousPatternPart
  )

  private def AnonymousPatternPart: Rule1[org.neo4j.cypher.internal.v3_6.expressions.AnonymousPatternPart] = rule (
      ShortestPathPattern
    | PatternElement ~~> ast.EveryPath
  )

  def ShortestPathPattern: Rule1[org.neo4j.cypher.internal.v3_6.expressions.ShortestPaths] = rule (
  ...
  def RelationshipsPattern: Rule1[org.neo4j.cypher.internal.v3_6.expressions.RelationshipsPattern] = rule {
    group(NodePattern ~ oneOrMore(WS ~ PatternElementChain)) ~~>> (ast.RelationshipsPattern(_))
  }.memoMismatches

  private def PatternElement: Rule1[org.neo4j.cypher.internal.v3_6.expressions.PatternElement] = rule (
      NodePattern ~ zeroOrMore(WS ~ PatternElementChain)
    | "(" ~~ PatternElement ~~ ")"
  )

  private def PatternElementChain: ReductionRule1[org.neo4j.cypher.internal.v3_6.expressions.PatternElement, org.neo4j.cypher.internal.v3_6.expressions.RelationshipChain] = rule("a relationship pattern") {
    group(RelationshipPattern ~~ NodePattern) ~~>> (ast.RelationshipChain(_, _, _))
  }

  private def RelationshipPattern: Rule1[org.neo4j.cypher.internal.v3_6.expressions.RelationshipPattern] = rule {
    (
  ...
  private def RelationshipDetail: Rule5[
      Option[ast.Variable],
      Option[org.neo4j.cypher.internal.v3_6.expressions.Variable],
      MaybeLegacyRelTypes,
      Option[Option[org.neo4j.cypher.internal.v3_6.expressions.Range]],
      Option[org.neo4j.cypher.internal.v3_6.expressions.Expression]] = rule("[") {
    (
        "[" ~~
  ...
  private def RelationshipTypes: Rule1[MaybeLegacyRelTypes] = rule("relationship types") (
    (":" ~~ RelTypeName ~~ zeroOrMore(WS ~ "|" ~~ LegacyCompatibleRelTypeName)) ~~>> (
      (first: org.neo4j.cypher.internal.v3_6.expressions.RelTypeName, more: List[(Boolean, org.neo4j.cypher.internal.v3_6.expressions.RelTypeName)]) => (pos: InputPosition) => {
        MaybeLegacyRelTypes(first +: more.map(_._2), more.exists(_._1))
      })
    | EMPTY ~ push(MaybeLegacyRelTypes())
  )

  private def LegacyCompatibleRelTypeName: Rule1[(Boolean, org.neo4j.cypher.internal.v3_6.expressions.RelTypeName)] =
    ((":" ~ push(true)) | EMPTY ~ push(false)) ~~ RelTypeName ~~>> (
      (legacy: Boolean, name: org.neo4j.cypher.internal.v3_6.expressions.RelTypeName) => (pos: InputPosition) => (legacy,name))

  private def MaybeVariableLength: Rule1[Option[Option[org.neo4j.cypher.internal.v3_6.expressions.Range]]] = rule("a length specification") (
      "*" ~~ (
  ...
  private def NodePattern: Rule1[org.neo4j.cypher.internal.v3_6.expressions.NodePattern] = rule("a node pattern") (
  ...
  private def MaybeNodeLabels: Rule1[Seq[org.neo4j.cypher.internal.v3_6.expressions.LabelName]] = rule("node labels") (
    WS ~ NodeLabels | EMPTY ~ push(Seq())
  )

  private def MaybeProperties: Rule1[Option[org.neo4j.cypher.internal.v3_6.expressions.Expression]] = rule("a property map") (
    optional(WS ~ (MapLiteral | Parameter | OldParameter))
  )
}

case class MaybeLegacyRelTypes(types: Seq[org.neo4j.cypher.internal.v3_6.expressions.RelTypeName] = Seq.empty, legacySeparator: Boolean = false)

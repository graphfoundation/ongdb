...
class AstGenerator(simpleStrings: Boolean = true, allowedVarNames: Option[Seq[String]] = None) {
  ...
  def _dbmsAction: Gen[DbmsAction] = oneOf(
    ...
    AllAliasManagementActions, CreateAliasAction, DropAliasAction, AlterAliasAction, ShowAliasAction,
  ...
  def _alterRemoteAlias: Gen[AlterRemoteDatabaseAlias] = for {
    ...
    driverSettings <- if (url.isEmpty && username.isEmpty && password.isEmpty) some(_optionalMapAsEither) else option(_optionalMapAsEither)

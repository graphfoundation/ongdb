...
case class Prettifier(
  ...
  def driverSettingsToString(settings: Map[String, Expression]): String =
    if (settings.nonEmpty) s" DRIVER ${settings.map({ case (s, e) => s"${backtick(s)}: ${expr(e)}" }).mkString("{", ", ", "}")}" else " DRIVER {}"
  ...
  def asString(adminCommand: AdministrationCommand): String =  {
    ...
    val commandString = adminCommand match {
      ...
      case x @ AlterRemoteDatabaseAlias(aliasName, targetName, ifExists, url, username, password, driverSettings) =>
        ...
        if (ifExists) s"${x.name} ${Prettifier.escapeName(aliasName)} IF EXISTS SET DATABASE$targetString$userString$passwordString$driverSettingsString"
        else s"${x.name} ${Prettifier.escapeName(aliasName)} SET DATABASE$targetString$userString$passwordString$driverSettingsString"

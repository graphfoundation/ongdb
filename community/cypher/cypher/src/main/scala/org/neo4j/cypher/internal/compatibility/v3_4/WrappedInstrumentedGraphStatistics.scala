...
class WrappedInstrumentedGraphStatistics(innerV3_4: GraphStatisticsV3_4, snapshotv3_6: MutableGraphStatisticsSnapshot) extends InstrumentedGraphStatisticsV3_4(innerV3_4, new MutableGraphStatisticsSnapshotV3_4()) {
  override def nodesWithLabelCardinality(labelId: Option[LabelIdV3_4]): CardinalityV3_4 =
    if (labelId.isEmpty) {
      snapshotv3_6.map.getOrElseUpdate(NodesWithLabelCardinality(None), 1)
    } else {
      snapshotv3_6.map.getOrElseUpdate(NodesWithLabelCardinality(labelId), inner.nodesWithLabelCardinality(labelId).amount)
    }
  ...
  override def cardinalityByLabelsAndRelationshipType(fromLabel: Option[LabelIdV3_4], relTypeId: Option[RelTypeIdV3_4], toLabel: Option[LabelIdV3_4]): CardinalityV3_4 =
    snapshotv3_6.map.getOrElseUpdate(
  ...
  override def indexSelectivity(index: IndexDescriptorV3_4): Option[SelectivityV3_4] = {
    ...
    snapshotv3_6.map.getOrElseUpdate(IndexSelectivity(index), selectivity.fold(0.0)(_.factor))
    selectivity
  }
  ...
  override def indexPropertyExistsSelectivity(index: IndexDescriptorV3_4): Option[SelectivityV3_4] = {
    ...
    snapshotv3_6.map.getOrElseUpdate(IndexPropertyExistsSelectivity(index), selectivity.fold(0.0)(_.factor))
    selectivity
  }

  override def nodesAllCardinality(): CardinalityV3_4 = snapshotv3_6.map.getOrElseUpdate(NodesAllCardinality, inner.nodesAllCardinality().amount)

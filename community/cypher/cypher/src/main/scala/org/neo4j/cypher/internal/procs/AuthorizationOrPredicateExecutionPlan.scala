...
case class AuthorizationOrPredicateExecutionPlan(securityAuthorizationHandler: SecurityAuthorizationHandler,
...
object AuthorizationOrPredicateExecutionPlan {
  private def buildMessage(securityAuthorizationHandler: SecurityAuthorizationHandler,
    ...
    val filteredErrorStates = if ( errorStates.exists(state => state._1 == PermissionState.EXPLICIT_DENY )) {
      errorStates.filterKeys(state => state != PermissionState.NOT_GRANTED)
    } else {
      errorStates
    }

    val errorMsgs = filteredErrorStates.map{ case (state, actions) => messageGenerator(state, actions.map(_._1))}.mkString("; ")

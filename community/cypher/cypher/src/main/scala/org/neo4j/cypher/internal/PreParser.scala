...
class PreParser(configuredVersion: CypherVersion,
  ...
  private final val ILLEGAL_PLANNER_VERSION_COMBINATIONS: Set[(CypherPlannerOption, CypherVersion)] =
    Set(
      (CypherPlannerOption.rule, CypherVersion.v3_3),
      (CypherPlannerOption.rule, CypherVersion.v3_4),
      (CypherPlannerOption.rule, CypherVersion.v3_5)
    )

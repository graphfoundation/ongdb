...
object helpers {
  ...
  def as3_6(pos: InputPositionV3_4): InputPosition = if(pos == null) null else InputPosition(pos.offset, pos.line, pos.column)

  def as3_6(planId: IdV3_4) : Id = Id(planId.x)

  def as3_6(plannerName: PlannerNameV3_4) : PlannerName = plannerName match {
  ...
  def as3_6(updateStrategy: UpdateStrategyV3_4): UpdateStrategy = updateStrategy match {
  ...
  def as3_6(periodicCommit: irV3_4.PeriodicCommit): irv3_6.PeriodicCommit = {
    irv3_6.PeriodicCommit(periodicCommit.batchSize)
  }

  def as3_6(cardinality: CardinalityV3_4) : Cardinality = {
    Cardinality(cardinality.amount)
  }

  def as3_6(statement: astV3_4.Statement) : astv3_6.Statement = StatementWrapper(statement)

  def as3_6(notification: nfV3_4.InternalNotification): nfv3_6.InternalNotification = notification match {
    case nfV3_4.DeprecatedStartNotification(position, alternativeQuery) => nfv3_6.DeprecatedStartNotification(as3_6(position), alternativeQuery)
    case nfV3_4.CartesianProductNotification(position, isolatedVariables) => nfv3_6.CartesianProductNotification(as3_6(position), isolatedVariables)
    case nfV3_4.LengthOnNonPathNotification(position) => nfv3_6.LengthOnNonPathNotification(as3_6(position))
    ...
    case nfV3_4.MissingLabelNotification(position, label) => compilerv3_6.MissingLabelNotification(as3_6(position), label)
    case nfV3_4.MissingRelTypeNotification(position, relType) => compilerv3_6.MissingRelTypeNotification(as3_6(position), relType)
    case nfV3_4.MissingPropertyNameNotification(position, name) => compilerv3_6.MissingPropertyNameNotification(as3_6(position), name)
    case nfV3_4.UnboundedShortestPathNotification(position) => nfv3_6.UnboundedShortestPathNotification(as3_6(position))
    case nfV3_4.ExhaustiveShortestPathForbiddenNotification(position) => compilerv3_6.ExhaustiveShortestPathForbiddenNotification(as3_6(position))
    case nfV3_4.DeprecatedFunctionNotification(position, oldName, newName) => nfv3_6.DeprecatedFunctionNotification(as3_6(position), oldName, newName)
    case nfV3_4.DeprecatedProcedureNotification(position, oldName, newName) => compilerv3_6.DeprecatedProcedureNotification(as3_6(position), oldName, newName)
    case nfV3_4.ProcedureWarningNotification(position, procedure, warning) => compilerv3_6.ProcedureWarningNotification(as3_6(position), procedure, warning)
    case nfV3_4.DeprecatedFieldNotification(position, procedure, field) => compilerv3_6.DeprecatedFieldNotification(as3_6(position), procedure, field)
    case nfV3_4.DeprecatedVarLengthBindingNotification(position, variable) => nfv3_6.DeprecatedVarLengthBindingNotification(as3_6(position), variable)
    case nfV3_4.DeprecatedRelTypeSeparatorNotification(position) => nfv3_6.DeprecatedRelTypeSeparatorNotification(as3_6(position))
  ...
  def as3_6(logicalPlanState: LogicalPlanStateV3_4) : LogicalPlanState = {
    val startPosition = logicalPlanState.startPosition.map(as3_6)
    // Wrap the planner name to correctly report version 3.4.
    val plannerName = PlannerNameWithVersion(as3_6(logicalPlanState.plannerName), v3_4.name)
    ...
    val (plan3_4, semanticTable3_4) = convertLogicalPlan(logicalPlanState, solveds3_4, cardinalities3_4, solveds3_6, cardinalities3_6)
    ...
    LogicalPlanState(logicalPlanState.queryText,
                     startPosition,
                     plannerName,
                     PlanningAttributes(solveds3_6, cardinalities3_6, providedOrders),
                     Some(as3_6(statement3_4)),
                     None,
                     logicalPlanState.maybeExtractedParams,
                     Some(semanticTable3_4),
                     None,
                     Some(plan3_4),
                     Some(logicalPlanState.maybePeriodicCommit.flatten.map(x => as3_6(x))),
                     Set.empty)
  }
  ...
  private def convertLogicalPlan(logicalPlanState: LogicalPlanStateV3_4,
                                 ...
                                 cardinalities3_6: CardinalitiesV3_6): (LogicalPlanv3_6, SemanticTablev3_6) = {

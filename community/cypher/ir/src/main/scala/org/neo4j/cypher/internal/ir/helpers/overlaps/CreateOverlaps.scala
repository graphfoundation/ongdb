...
object CreateOverlaps {
  ...
  private def extractNodePredicates(expressions: Seq[Expression]): (Seq[Expression], NodePredicates) =
    partitionMap(expressions.flatMap(Expressions.splitExpression))(expression =>
  ...
  def partitionMap[A, B, C](values: Seq[A])(f: A => Either[B, C]): (Seq[B], Seq[C]) = {
    val lefts = Seq.newBuilder[B]
    val rights = Seq.newBuilder[C]
    values.foreach { value =>
      f(value) match {
        case Left(left) => lefts += left
        case Right(right) => rights += right
      }
    }

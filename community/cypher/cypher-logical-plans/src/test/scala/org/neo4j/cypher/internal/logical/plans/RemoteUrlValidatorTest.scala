...
class RemoteUrlValidatorTest extends CypherFunSuite {
  ...
  (Seq("http", "https", "bolt", "bolt+s", "bolt+ssc").map(_ + url) :+ noSchemeUrl).foreach {
    url =>
      ...
      test(s"validator should throw exception for invalid url '$url' with insecure schemes and no security requirement") {
        val Some(exception) = RemoteUrlValidator.assertValidRemoteDatabaseUrl(url, secure = false)
        exception shouldBe a[InvalidArgumentException]
        exception should have message MessageUtil.invalidScheme(url, validSchemes.asJava)
      }
  }

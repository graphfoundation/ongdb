...
{
    @Description( "Enable a remote shell server which ONgDB Shell clients can log in to. Only applicable to `ongdb-shell`." )
    ...
    @Description( "Remote host for shell. By default, the shell server listens only on the loopback interface, " +
            "but you can specify the IP address of any network interface or use `0.0.0.0` for all interfaces. Only applicable to `ongdb-shell`." )
    ...
    @Description( "The port the shell will listen on. Only applicable to `ongdb-shell`." )
    public static final Setting<Integer> remote_shell_port =
            buildSetting( "dbms.shell.port", INTEGER, "1337" ).constraint( port ).build();

    @Description( "Read only mode. Will only allow read operations. Only applicable to `ongdb-shell`." )
    public static final Setting<Boolean> remote_shell_read_only = setting( "dbms.shell.read_only", BOOLEAN, FALSE );

    @Description( "The name of the shell. Only applicable to `ongdb-shell`." )

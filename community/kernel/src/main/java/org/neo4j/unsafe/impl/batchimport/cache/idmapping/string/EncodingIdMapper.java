 ...
 * - input id:
 *       An id coming from the user that is associated with a ONgDB id by calling {@link #put(Object, long, Group)}.
 *       the first argument is the id that the user specified, the second is the ONgDB id that user id will
 *       be associated with.
 * - encoder:
 ...
 * - data cache:
 *       An array of eIds. eIds are added in the order of ONgDB ids, i.e. in the order in which they are put.
 * - tracker cache:

...
class NodeIndexTxStateUpdaterTest extends IndexTxStateUpdaterTestBase
{
    ...
    private final IndexDescriptor indexOn1_1_new = TestIndexDescriptorFactory.forLabel( LABEL_ID_1, PROP_ID_1, NEW_PROP_ID );
    private final IndexDescriptor uniqueOn2_2_3 = TestIndexDescriptorFactory.uniqueForLabel( LABEL_ID_2, PROP_ID_2, PROP_ID_3 );
    private final List<IndexDescriptor> indexes = Arrays.asList( indexOn1_1, indexOn2_new, uniqueOn1_2, indexOn1_1_new, uniqueOn2_2_3 );
    ...
    {
        // WHEN
        indexTxUpdater.onPropertyAdd( node, propertyCursor, node.labels().all(), UN_INDEXED_PROP_ID, PROPS, Values.of( "whAt" ) );
        indexTxUpdater.onPropertyRemove( node, propertyCursor, node.labels().all(), UN_INDEXED_PROP_ID, PROPS, Values.of( "whAt" ) );
        indexTxUpdater.onPropertyChange( node, propertyCursor, node.labels().all(), UN_INDEXED_PROP_ID, PROPS, Values.of( "whAt" ), Values.of( "whAt2" ) );
    ...
    {
        // WHEN
        indexTxUpdater.onPropertyChange( node, propertyCursor, node.labels().all(), PROP_ID_2, PROPS, Values.of( "hi2" ), Values.of( "new2" ) );

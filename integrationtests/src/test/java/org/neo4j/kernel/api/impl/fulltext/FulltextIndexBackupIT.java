...
/**
 * Verifies fulltext indexes survive full and incremental online backup restore.
 */
public class FulltextIndexBackupIT
{
    ...
    @Rule
    public final RuleChain rules = RuleChain.outerRule( suppressOutput ).around( testDirectory ).around( cleanup );
    ...
    public void setUpPorts()
    {
        ...
        GraphDatabaseBuilder builder = new TestEnterpriseGraphDatabaseFactory()
                .newEmbeddedDatabaseBuilder( testDirectory.databaseDir() );
        builder.setConfig( MetricsSettings.metricsEnabled, Settings.FALSE );
    ...
    {
        // Online backup spins a temp recovery DB; metrics must stay off so kernel extensions do not
        // require StoreEntityCounters that the temp facade does not satisfy.
        return Config.defaults( MetricsSettings.metricsEnabled, Settings.FALSE );
    }
    ...
    public void fulltextIndexesMustBeTransferredInBackup()
    {
        initializeTestData();
        verifyData( db );
        File backup = testDirectory.databaseDir( "backup" );
        OnlineBackup.from( "127.0.0.1", backupPort ).backup( backup, backupConfig() );
        db.shutdown();
    ...
    {
        return (GraphDatabaseAPI) cleanup.add( new TestEnterpriseGraphDatabaseFactory()

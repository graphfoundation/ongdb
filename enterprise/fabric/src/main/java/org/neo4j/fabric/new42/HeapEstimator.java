...
import static org.neo4j.fabric.new42.RuntimeInternals.STRING_VALUE_ARRAY;
import static org.neo4j.fabric.new42.RuntimeInternals.stringBackingArraySize;
...
public final class HeapEstimator
{
    ...
    static
    {
        ...
        if ( RuntimeInternals.DEBUG_ESTIMATIONS )
        {
            System.err.println( String.format( "### %s static values: ###%n" +
                                               ...
                                               "  LONG_SIZE=%d%n" +
                                               "  STRING_SIZE=%d%n" +
                                               "  STRING_VALUE_ARRAY=%s%n", HeapEstimator.class.getName(), OBJECT_ALIGNMENT_BYTES, OBJECT_REFERENCE_BYTES,
                                               OBJECT_HEADER_BYTES,
    ...
    /**
     * Returns the estimated size of the provided map (assuming it is a {@link HashMap}). This only calculates the size of the map structure, the entries are
     * not traversed and needs to be tracked separately.
     *
        ...
        for ( ; clazz != null; clazz = clazz.getSuperclass() )
        {
            for ( Field f : clazz.getDeclaredFields() )
            {
                if ( !Modifier.isStatic( f.getModifiers() ) )
                {
                    ...
                    size = max( size, UnsafeUtil.getFieldOffset( f.getClass(), f.getName() ) + fieldSize );
                }
            }
        }
        return alignObjectSize( size );

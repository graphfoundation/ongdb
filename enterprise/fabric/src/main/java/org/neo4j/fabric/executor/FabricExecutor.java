...
import org.neo4j.fabric.new42.RoutingContext;
    ...
    private class FabricStatementExecution
    {
        ...
        private TransactionMode getTransactionMode( QueryType queryType, String graph )
        {
            ...
            else
            {
                if ( queryMode == AccessMode.WRITE )
                {
                    // This should be  AccessMode( ClientError, "The request could not be completed due to access mode violation" ), which is in 4.2
                    throw new FabricException( Status.Statement.SemanticError, "Writing in read access mode not allowed. Attempted write to %s", graph );
                }
                else
                {
                    return TransactionMode.DEFINITELY_READ;

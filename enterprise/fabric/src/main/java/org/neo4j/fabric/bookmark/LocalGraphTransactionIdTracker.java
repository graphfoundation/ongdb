...
public class LocalGraphTransactionIdTracker
{
    ...
    /**
     * Unlike {@link #awaitSystemGraphUpToDate(long)}, the caller does not know which graph is System one and this method will find it in the the submitted
     * Graph UUID to transaction Id map.
     */
    ...
    private NamedDatabaseId getNamedDatabaseId( Location.Local location )
    {
        ...
        {
            // this can only happen when the db has just been deleted or someone tempered with a bookmark
            throw new IllegalArgumentException( "A local graph could not be mapped to a db" );
        }

 ...
 * <p>
 * Fabric concurrent parts are not independent. A failure in one record stream can cause secondary failures in other streams for instance if the streams are
 * using the same transaction. Due to the concurrent nature, there might be a race between the primary exception and secondary exceptions. The user should be
 * always presented with the primary exception.
 */

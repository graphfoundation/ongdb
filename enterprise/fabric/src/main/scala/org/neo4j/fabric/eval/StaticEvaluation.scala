...
import org.neo4j.fabric.new42.CypherRow
...
object StaticEvaluation {
  ...
  private trait EmptyQueryContext extends QueryContext {
    ...
    def nodeCursor(): NodeCursor = notAvailable()

    def traversalCursor(): RelationshipTraversalCursor = notAvailable()
    ...
    override def indexSeek[RESULT <: AnyRef](index: IndexReadSession,
                                             ...
                                             queries: Seq[IndexQuery]): NodeValueIndexCursor = notAvailable()
    ...
    def getNodesByLabel(id: Int, indexOrder: IndexOrder): Iterator[NodeValue] = notAvailable()

    def getNodesByLabelPrimitive(id: Int, indexOrder: IndexOrder): LongIterator = notAvailable()
    ...
    def nodeHasCheapDegrees(node: Long, nodeCursor: NodeCursor): Boolean = notAvailable()
    ...
    override def variableLengthPathExpand(realNode: Long,
                                          ...
                                          relTypes: Seq[String]): Iterator[Path] = notAvailable()
    ...
    def allShortestPath(left: Long,
                        ...
                        pathPredicate: KernelPredicate[Path],
                        filters: Seq[KernelPredicate[Entity]],
                        memoryTracker: MemoryTracker): Iterator[Path] = notAvailable()
    ...
    def relationshipTypeName(typ: Int): String = notAvailable()
  ...
  class StaticEvaluator(proceduresSupplier: Supplier[GlobalProcedures]) extends SimpleInternalExpressionEvaluator {
    ...
    def evaluate(expression: Expression, params: MapValue, context: CypherRow): AnyValue = {
      try {
        // Works with 4.2 but we need ExecutionContext in 4.0.x
        //super.evaluate(expression, params, context)
        super.evaluate(expression, params, ExecutionContext.empty)
      } catch {

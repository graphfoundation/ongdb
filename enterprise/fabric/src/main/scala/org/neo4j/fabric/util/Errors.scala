...
object Errors {
  ...
  def openCypherUnexpected(exp: String,
                           got: String,
                           in: String,
                           pos: InputPosition): Nothing = openCypherInvalid(SemanticError(s"Expected: $exp, got: $got, in: $in", pos))

  def openCypherUnexpected(exp: String, got: ASTNode): Nothing = openCypherUnexpected(exp, got.position)

  def openCypherUnknownFunction(qualifiedName: String,
                                pos: InputPosition): Nothing = openCypherFailure(SemanticError(s"Unknown function '$qualifiedName'", pos))
  ...
  def ddlNotSupported(ddl: CatalogDDL) = throw new DatabaseAdministrationException(
    s"This is an administration command and it should be executed against the system db: ${ddl.name}")

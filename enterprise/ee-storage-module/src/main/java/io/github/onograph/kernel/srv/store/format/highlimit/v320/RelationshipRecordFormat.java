...
class RelationshipRecordFormat extends AbstractHighLimitRecordFormat<RelationshipRecord> {
  ...
  private static void _refirenreForRePa(long hdrLng, boolean isBeingUsed, PageCursor _pageCursor,
    ...
    long _lgFnm = (mdrSrt & 1L) << 32;

    long _lgNs = (long) _pageCursor.getInt() & 4294967295L;

    long _lgMns = (mdrSrt & 2L) << 31;

    long _lgFpr = (long) _pageCursor.getInt() & 4294967295L;
    ...
    _relationshipRecord.initialize(isBeingUsed, longFromIntAndMod(nxt, _lgMpn),
        longFromIntAndMod(_lgNf, _lgFnm),
        longFromIntAndMod(_lgNs, _lgMns), tNme, longFromIntAndMod(_lgFpr, _lgMrpf),
        longFromIntAndMod(_lgRnf, _lgMrnf), longFromIntAndMod(_lgRps, _lgMrps),
        longFromIntAndMod(_lgRns, _lgMrns), has(hdrLng, 8), has(hdrLng, 16));
  }
  ...
  private static void _wrfrerdForRePa(PageCursor _pageCursor,
    ...
    long _lgFpr = _relationshipRecord.getFirstPrevRel();

    long _lgMrpf = _lgFpr == NUL ? 0L : (_lgFpr & 4294967296L) >> 30;
  ...
  protected void doreieaForREPa(int rLeng, boolean isBeingUsed, long hdrLng, PageCursor _pageCursor,
    ...
    if (_relationshipRecord.isUseFixedReferences()) {
      ...
      int _iThw = _pageCursor.getByte() & 255;
      tNme = _iThw << 16 | tNme;

...
abstract class AbstractHighLimitRecordFormat<RECORD extends AbstractBaseRecord> extends
  ...
  private static String _ilasoyrrmgString(long sId) {
    return io.github.onograph.I18N.format(
        "io.github.onograph.kernel.srv.store.format.highlimit.v310.AbstractHighLimitRecordFormat.ilasoyrrmgString",
        sId);
  }
  ...
  protected abstract void doreieaForREPa(int _iVar, boolean _isV, long _lgVar,
  ...
  protected abstract boolean isCauefdrrsForRE(int _iV, RECORD _rcrVa);
  ...
  protected void makausForPaRE(PageCursor _pageCursor, int rLeng, RECORD _rcrRcr, int rpSize)
    ...
    if (_rcrRcr.hasSecondaryUnitId()) {
      ...
      if (!_pageCursor.next(_lgRsfip)) {
        ...
        throw new UnderlyingStorageException("*** Error: 33bdf0b3-b515-4984-9f65-085d0a485166");
      }
  ...
  public void read(RECORD _rcrRcr, PageCursor _cpPageCursor, RecordLoad mdstr, int rLeng,
    ...
    try {
      ...
      if (!_isUrf) {
        ...
        if (mdstr.failOnNonFirstUnit()) {
          _cpPageCursor.setCursorException(io.github.onograph.I18N.format(
              "io.github.onograph.kernel.srv.store.format.highlimit.v310.AbstractHighLimitRecordFormat.setCursorException"));
        }
      ...
      if (!(!_csPageCursor.next() | ofst < 0)) {
        ...
        int _iPs = rLeng - (_cpPageCursor.getOffset() - _iOsp);

        int _iSs = rLeng - 1;

        PageCursor _cmoiePageCursor = CompositePageCursor.compose(_cpPageCursor, _iPs,
            _csPageCursor, _iSs);

...
public class PropertyOneByteHeaderRecordFormat extends
    BaseOneByteHeaderRecordFormat<PropertyRecord> {
  ...
  public PropertyOneByteHeaderRecordFormat() {
  ...
  private static void _refirenreForPrPa(boolean isBeingUsed, PageCursor _pageCursor,
    ...
    long _lgPm = (long) _pageCursor.getShort() & 65535L;
    ...
    _propertyRecord.initialize(isBeingUsed, BaseRecordFormat.longFromIntAndMod(_lgPp, _lgPm << 32),
  ...
  private static void _wrfrerdForPrPa(PageCursor _pageCursor, PropertyRecord _propertyRecord) {
    ...
    short _mpShort =
        _propertyRecord.getPrevProp() == AbstractHighLimitRecordFormat.NUL ? 0 : (short) ((int) (
            (_propertyRecord.getPrevProp() & 281470681743360L) >> 32));

    short _mnShort =
        _propertyRecord.getNextProp() == AbstractHighLimitRecordFormat.NUL ? 0 : (short) ((int) (
            (_propertyRecord.getNextProp() & 281470681743360L) >> 32));
  ...
  public void read(PropertyRecord _propertyRecord, PageCursor _pageCursor, RecordLoad mdstr,
    ...
    if (mdstr.shouldLoad(isBeingUsed)) {
      ...
      _propertyRecord.setUseFixedReferences(_isUfr);
      if (_iCb > _propertyRecord.getBlockCapacity()
        ...
        _pageCursor.setCursorException(io.github.onograph.I18N.format(
            "io.github.onograph.kernel.srv.store.format.highlimit.PropertyOneByteHeaderRecordFormat.setCursorException"));
        return;
      }

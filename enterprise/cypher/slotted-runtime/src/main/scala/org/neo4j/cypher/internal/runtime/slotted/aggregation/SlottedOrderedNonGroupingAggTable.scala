...
class SlottedOrderedNonGroupingAggTable(slots: org.neo4j.cypher.internal.physicalplanning.SlotConfiguration,
                                        orderedGroupingColumns: GroupingExpression,
                                        aggregations: Map[Int, AggregationExpression],
                                        state: QueryState) extends SlottedNonGroupingAggTable(slots, aggregations, state) with OrderedChunkReceiver {
...
object SlottedOrderedNonGroupingAggTable {
  ...
  case class Factory(val slots: SlotConfiguration,
                     val orderedGroupingColumns: GroupingExpression,
                     val aggregations: Map[Int, AggregationExpression]) extends OrderedAggregationTableFactory {
    override def table(state: QueryState,
                       executionContextFactory: ExecutionContextFactory): AggregationPipe.AggregationTable with OrderedChunkReceiver = {
      ???
    }

...
trait CypherReductionSupport extends CypherTestSupport with GraphIcing {
  ...
  private def executeInTx(query: String, statement: Statement, parsingBaseState: BaseState, implicitTx: InternalTransaction, enterprise: Boolean): RewindableExecutionResult = {
    ...
    val runtimeContextCreator = if (enterprise)
      EnterpriseRuntimeContextCreator(GeneratedQueryStructure, NullLog.getInstance(), CypherReductionSupport.config, dispatcher = null)
    else

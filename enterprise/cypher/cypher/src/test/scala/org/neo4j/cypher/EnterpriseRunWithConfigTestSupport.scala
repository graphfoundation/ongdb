...
import org.neo4j.test.EnterpriseTestGraphDatabaseFactory

import scala.collection.JavaConverters._

trait EnterpriseRunWithConfigTestSupport {
  def runWithConfig(m: (Setting[_], String)*)(run: GraphDatabaseCypherService => Unit) = {
    ...
    val graph = new EnterpriseTestGraphDatabaseFactory().newImpermanentDatabase(storeDir, config)
    try {

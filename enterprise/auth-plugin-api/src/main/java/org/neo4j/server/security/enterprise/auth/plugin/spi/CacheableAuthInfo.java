...
/**
 * A cacheable object that can be returned as the result of successful authentication by an {@link AuthPlugin}.
 *
 ...
 * If you instead want to manage this yourself you need to use the separate interfaces {@link AuthenticationPlugin} and {@link AuthorizationPlugin} together
 * with a {@link CustomCacheableAuthenticationInfo} result.
 *
...
public interface CacheableAuthInfo extends AuthInfo
{
    ...
    /**
     * Should return a principal that uniquely identifies the authenticated subject within this auth provider. This will be used as the cache key, and needs to
     * be matcheable against a principal within the auth token map.
     *
    ...
    /**
     * Should return credentials that can be cached, so that successive authentication attempts could be performed against the cached authentication info from a
     * previous successful authentication attempt.
     *
